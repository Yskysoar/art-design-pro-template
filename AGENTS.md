...
- 当前社交关注与聊天模块已完成第一轮开发：复用 `sys_user`；顶部栏聊天进入隐藏路由 `/social/chat`；已实现关注、粉丝、互关、拉黑、HTTP 私信、3 条等待回复额度、未读已读和敏感词拦截；已新增 `social_follow`、`social_block`、`social_conversation`、`social_message` 表结构、Mock SQL、前端聊天页和 `SocialServiceImplTest`；WebSocket 作为后续升级预留。
