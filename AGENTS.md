...
- 当前文件资源管理模块已完成第一版：基于 `file_resource` 统一查看文章上传、富文本上传和社交上传资源台账；新增 `/api/file-resource/list`、`DELETE /api/file-resource/{id}`、系统管理“文件资源”页面、`system:file:manage` 权限码和 `FileResourceManageServiceImplTest`。第一版只删除未引用资源台账，不删除磁盘物理文件。
- 当前通知与消息中心模块已完成第一版：新增 `notification_message` 表、`/api/notifications`、未读数、单条/全部已读；顶部通知弹层已从模板静态数据切换到真实接口；关注、私信、评论回复会写入通知；新增隐藏路由 `/notification/center` 和 `NotificationServiceImplTest`。WebSocket 实时推送、系统公告后台发布为后续项。
- 当前内容治理与举报审核模块已完成第一版：新增 `content_report` 表、`/api/moderation/reports` 创建/分页/处理接口、内容治理“举报审核”页面、`moderation:report:manage` 权限码和 `ContentReportServiceImplTest`。第一版只记录和处理举报状态，不自动下架、隐藏、删除或封禁。
...
- `Docs/9.文件资源管理模块.md` 维护统一文件资源台账、引用识别、资源筛选和未引用资源删除。
- `Docs/10.通知与消息中心模块.md` 维护通知台账、顶部铃铛、消息中心和互动通知事件。
- `Docs/11.内容治理与举报审核模块.md` 维护举报台账、审核处理、权限规则和后续治理动作规划。
